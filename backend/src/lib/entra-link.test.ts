...
function mapEmployee(row: EmployeeRow): Employee {
  return {
    ...
    hireDate:
      dateValue(row.hire_date) ?? "",

    baseSalary:
      Number(row.base_salary ?? 0),

    status:
      row.status as Employee["status"],
    ...
    civility:
      (row.civility ?? "M") as Employee["civility"],
    ...
    contractHours:
      Number(row.contract_hours ?? 151.67),

    pasRate:
      Number(row.pas_rate ?? 0),

    mealTicket5:
      Number(row.meal_ticket_5 ?? 0),

    mealTicket1650:
      Number(row.meal_ticket_1650 ?? 0),

    contractEndDate:
      dateValue(row.contract_end_date),

    entraObjectId:
      row.entra_object_id ?? undefined,

    entraUserPrincipalName:
      row.entra_user_principal_name ?? undefined,

    directoryRole:
      row.directory_role ?? undefined,
  };
}
...
function identityFromInput(
  ...
  const email =
    normalizeMicrosoftEmail(input.email);
...
export async function findEmployeeForMicrosoft(
  ...
  const email =
    normalizeMicrosoftEmail(identity.email);

  const result =
    await queryPostgres<EmployeeRow>(
      `
      SELECT *
      FROM employees
      WHERE
        ...
        (
          $2 <> ''
          AND LOWER(entra_user_principal_name) = LOWER($2)
        )
        OR
        (
          $2 <> ''
...
export async function provisionMicrosoftProfile(
  ...
  /*
   * Recherche d'abord l'utilisateur dans PostgreSQL.
   */
  const current =
    await findEmployeeForMicrosoft({
      id: identity.oid,
      email: identity.email,
    });

  /*
   * Si l'employé existe déjà, on synchronise
   * les informations provenant de Microsoft Entra ID.
   */
  if (current) {
    const result =
      await queryPostgres<EmployeeRow>(
        ...
        [
          ...
          identity.oid ||
            current.entraObjectId ||
            null,

          identity.email ||
            current.entraUserPrincipalName ||
            null,
  ...
  /*
   * Sinon création automatique de la fiche
   * à la première connexion Microsoft.
   */
  const employeeId =
    randomUUID();
